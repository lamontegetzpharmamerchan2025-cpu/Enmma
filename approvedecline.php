...
<body> 
      ...
      <section class="container addproducts ml-auto text-black">
        <h1 class="m-0">APPROVE AND DECLINE</h1>
        <br>
        <table class="table">
            ...
            <tbody>         
            <?php
                    $tsql = sqlsrv_query($conn,"SELECT * from itemtaborig where itemstatus='NOTPOST'");
                     while($row = sqlsrv_fetch_array($tsql)){
                      ...
                      echo "
                        <tr>
                          ...
                          <td>
                          <center>
                          <button class='btn btn-success approve' data-id='".$row['itemid']."'>APPROVE</button>
                          <button class='btn btn-warning decline' data-id='".$row['itemid']."'>DECLINE</button>
                          </center>
                          </td>                         
                          </tr>
                      ";
...
<?php include 'includes/approve_form.php';?>
</div>
<?php include 'includes/decline_form.php';?>
<script> 
$(function(){
    ...
    $('#decline').modal('show');
    var id = $(this).data('id');
    $('#itemid2').val($(this).data('id'));
  });
  $('.approve').click(function(e){
    e.preventDefault();
    $('#approve').modal('show');
    var id = $(this).data('id');
    $('#itemid1').val($(this).data('id'));
  });
});
function getRow(id){
  $.ajax({
    type: 'POST',
    url: 'approvedecline_row.php',
    data: {id:id},
    dataType: 'json',
